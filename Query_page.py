...
if questions == "1. Names of all the videos and their channels":
    ...
    query = """
                select vt.channel_name, sum(vt.view_count) as views_count from video_table as vt join playlist_table as pt
                on (vt.channel_id = pt.channel_id) group by vt.channel_name order by views_count desc limit 10
            """
    mycursor.execute(query)
    table = mycursor.fetchall()
    df3 = pd.DataFrame(table, columns=["channel name", "view counts"])
    st.table(df3)
    st.header(':violet[Visualization: Top 10 most viewed videos')
    fig = px.bar(df3,
        y = 'channel name',
        x = 'view counts',
        orientation = 'h'
    ...
    query = """
                select vt.channel_name, sum(vt.view_count) as views_count from video_table as vt join playlist_table as pt
                on (vt.channel_id = pt.channel_id) group by vt.channel_name order by views_count
            """
    mycursor.execute(query)
    table = mycursor.fetchall()
    ...
    mycursor.execute(query)
    table = mycursor.fetchall()
    df8 = pd.DataFrame(table, columns=["Channel published in the year 2022"])
    st.header(':violet[Table]')
    st.table(df8)
